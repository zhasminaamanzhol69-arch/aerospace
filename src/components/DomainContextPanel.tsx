...
const text: Record<Language, Record<VehicleDomain, {
  ...
  ru: {
    aviation: {
      ...
      body: {
        design: 'Расчёт опирается на аэродинамику, нагрузку на крыло/диск, ветер, взлётную массу и запас энергии для полётной миссии.',
        manufacturing: 'Материалы и сборка подбираются для лёгких авиационных узлов, композитных крыльев и фюзеляжа БПЛА.',
        operations: 'Телеметрия оценивает батарею, ветер, температуру двигателей, уровень сигнала связи и безопасность аварийного возврата.',
      },
      standards: 'FAA Part 107, ICAO, ISO 9001/AS9100, ГОСТ по авиационному контролю',
    },
    spacecraft: {
      title: 'Космический аппарат: кубсат / спутник',
      body: {
      ...
      standards: 'ECSS-E/ST, ECSS-Q-ST-70C, NASA-STD-5001, технические стандарты NASA',
    },
  },
  en: {
    aviation: {
...
export function DomainContextPanel({ stage, domain }: Props) {
  ...
  return (
    <section className="domain-context">
      <div>
        <p className="eyebrow">{language === 'ru' ? 'Логика типа аппарата' : language === 'kk' ? 'Аппарат түрінің логикасы' : 'Vehicle Domain Logic'}</p>
        <h2>{copy.title}</h2>
        <p>{copy.body[stage]}</p>
      </div>
