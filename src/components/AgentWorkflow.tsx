...
const text: Record<Language, {
  ...
  kk: {
    ...
    steps: {
      design: [
        { title: 'Миссияны талдайды', result: 'Payload, MTOW, орбита/биіктік және габарит қабылданды.' },
        { title: 'Геометрияны есептейді', result: 'S, b, L/D және жүктеме бағаланды.' },
        { title: 'Архитектураны таңдайды', result: 'Fixed Wing, VTOL, Multirotor немесе Satellite салыстырылды.' },
      ],
      manufacturing: [
        { title: 'Материалды талдайды', result: 'CFRP, алюминий, титан немесе 3D материал таңдалды.' },
        { title: 'Процесті бағалайды', result: 'ЧПУ, автоклав, инфузия немесе 3D өндіріс тексерілді.' },
        { title: 'Сапа бақылауын таңдайды', result: 'Допуск, lead time және NDT әдісі ұсынылды.' },
      ],
      operations: [
        { title: 'Телеметрияны оқиды', result: 'SoH, RSSI, температура және latency қабылданды.' },
        { title: 'Digital Twin жаңартады', result: 'Виртуалды модель аппарат күйін қайталады.' },
        { title: 'ТО және RTH ұсынады', result: 'Аномалия, ресурс және қауіпсіздік шектеуі анықталды.' },
      ],
    },
  },
  ru: {
    ...
    steps: {
      design: [
        { title: 'Анализирует миссию', result: 'Payload, MTOW, высота/орбита и габариты приняты.' },
        { title: 'Считает геометрию', result: 'Оценены S, b, L/D и нагрузка на крыло/диск.' },
        { title: 'Выбирает архитектуру', result: 'Сравнены Fixed Wing, VTOL, Multirotor и Satellite.' },
      ],
      manufacturing: [
        { title: 'Анализирует материал', result: 'Проверены CFRP, алюминий, титан и 3D-печать.' },
        { title: 'Оценивает процесс', result: 'Сравнены ЧПУ, автоклав, инфузия и additive manufacturing.' },
        { title: 'Назначает контроль качества', result: 'Рассчитаны допуск, lead time и выбран NDT.' },
      ],
      operations: [
        { title: 'Читает телеметрию', result: 'Приняты SoH, RSSI, температура, latency и flight hours.' },
        { title: 'Обновляет Digital Twin', result: 'Виртуальная модель повторяет текущее состояние аппарата.' },
        { title: 'Назначает ТО и RTH', result: 'Определены аномалии, ресурс и ограничения эксплуатации.' },
      ],
    },
  },
  en: {
...
function buildDomainSteps(steps: Array<{ title: string; result: string }>, stage: EngineeringStage, domain: VehicleDomain, language: Language) {
  ...
  const resultByStage: Record<Language, Record<EngineeringStage, string[]>> = {
    kk: {
      design: ['Payload, орбита, күн панельдері және габарит қабылданды.', 'Жылу режимі, радиация қоры және ұшыру жүктемесі бағаланды.', 'CubeSat, LEO/SSO/GEO немесе hosted payload салыстырылды.'],
      ...
      operations: ['Orbit mode, power balance, link және thermal cycling қабылданды.', 'Цифрлық модель спутниктің орбиталық күйін қайталады.', 'Қуат қоры, байланыс тәуекелі және ресурс шектеуі анықталды.'],
    },
    ru: {
      design: ['Payload, орбита, солнечные панели и габариты приняты.', 'Оценены тепловой режим, радиационная стойкость и пусковые нагрузки.', 'Сравнены CubeSat, LEO/SSO/GEO и hosted payload.'],
      manufacturing: ['Проверены материал, дегазация и радиационная стойкость по ECSS-Q.', 'Сравнены DMLS, ЧПУ и космический композитный процесс.', 'Назначены вакуумный контроль, термоциклы и NDT.'],
      operations: ['Приняты orbit mode, power balance, link и thermal cycling.', 'Digital Twin повторяет орбитальное состояние спутника.', 'Определены запас энергии, риск связи и ресурсные ограничения.'],
    },
    en: {
